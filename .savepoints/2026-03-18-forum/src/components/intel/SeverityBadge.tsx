...
export function SeverityBadge({
  ...
  return (
    <span
      className={`inline-flex items-center rounded border font-medium uppercase tracking-wide ${STYLES[severity]} ${small ? "px-1.5 py-0.5 text-[9px]" : "px-2 py-0.5 text-[10px]"} ${pulse && severity === "CRITICAL" ? "animate-pulse" : ""}`}
    >
      {t(severity)}
    </span>
  );
